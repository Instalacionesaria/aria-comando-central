...
**Rastro** · `stepDetail()` (`lib/aios/conversion.js:395-555`), abierto por `openStep()` (`:557-566`)
desde cualquier `[data-step]` de la vista (`:568`). El subtítulo del grupo lo anuncia: *«abre un paso
...
**Rastro** · `ZONES` (`conversion.js:116-123`), seis zonas con scroll y clicks, actual y previo.
Leyenda `Menos … Más clicks` (`:354`) y rótulo lateral `Hasta dónde llega el scroll` (`:357`).
...
(`conversion.js:338-341`).
**Lo que sobrevive**: nada construible hoy. Va a `11-LOS-DOS-SUBMODULOS.md` como hueco declarado.

### CV4-04 · `VSL` — la curva de retención

**Rastro** · Un SVG dibujado a mano con **89 literales de geometría** (`conversion.js:436-451`), dos
círculos rojos marcando caídas en coordenadas fijas (`:446-447`), el mapa de calor `VHEAT = [100, 88,
82, 74, 56, 51, 47, 44, 42, 41]` (`:125`) y dos filas de caída con sus momentos exactos:
`0:18 · 82% → 74% · −8 pts` y `0:27 · 74% → 56% · −18 pts` (`:453-456`).
**Estado** · **Sin fuente, y es el hueco más caro.** Ver `CV14-07`. La normalización del calor usa
además un máximo fijo de 20 puntos (`:365`).
...
**Rastro** · `FIELDS` (`conversion.js:134-137`), cinco campos con «llegan» y «pierde».
...
**Rastro** · Dos filas (`conversion.js:493-498`): `Asistencia esperada · Según consumo del VSL y
...
**Rastro** · `conversion.js:543-546`, con un array literal escrito en línea.
...
**Rastro** · `stepDetail('calificados')` (`conversion.js:504-528`).
**Estado** · **Inalcanzable.** `'calificados'` no está en `STEPS` (`:34-40`) y ningún nodo de la
pantalla emite `data-step="calificados"`. Las seis cajas, la `Lectura` y las tres filas de «Por dónde
entraron» **no se dibujan nunca**.

Y declara una **quinta fuente, `CRM`** (`:506`), que no figura entre los chips del encabezado
...
**Rastro** · `obsBlock()` (`conversion.js:374-393`), con el contador `«N a revisar · M a favor»`
(`:387-388`) y, por fila, `−N contactos` a la derecha (`:385`).
...
`x.loss × FACTOR[periodo]` (`:311`), sin relación explícita con las sesiones del período. Un `−64
...
### CV4-10 · `closeReco()` no existe

**Rastro** · `conversion.js:618`:

```js
el.onclick = ()=>{ closeReco(); openStep(el.dataset.goto); };
```

**Estado** · **`closeReco` no está definida en ningún archivo de la aplicación** — ni en `lib/`, ni
en `components/`, ni en `app/`, ni en `scripts/`. `conversion.js` no tiene una sola sentencia
`import`, y `lib/aios/shell.js` exporta `cerrarElModal`, no `closeReco`. La única definición vive en
el prototipo `aios-command-center_1.html`, que no se carga.

**Consecuencia**: clicar cualquiera de las tres filas del plan de acción lanza `ReferenceError`, el
`openStep()` posterior **nunca se ejecuta** y el modal se queda encima. Es el mismo modo de fallo que
bloqueó el borrado en Creative, en la dirección contraria: allá **faltaba el cierre**, acá la llamada
apunta a un nombre fantasma.

Los cierres de los dos overlays ya están centralizados en `lib/aios/shell.js:187-201` desde la etapa 0
de Creative, y hay una prueba que los cubre (`pruebas/codigo/156-cierre-de-los-overlays.test.ts:50`)
— pero comprueba el texto de `shell.js`, **no esta llamada**.
