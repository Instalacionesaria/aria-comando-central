...
**La pantalla.** `components/views/CreativeView.jsx` (99 líneas) es sólo el maquetado: encabezado «Creative — Qué funciona, qué no, y por qué» (líneas 17-24), un botón «Plan de acción» (línea 30), tres botones de periodo (líneas 31-40) y seis contenedores vacíos —`statRow`, `sortSeg`, `goodGrid`, `badGrid`— que llena JavaScript. No hace un solo pedido al servidor.
...
- **¿Se puede reemplazar hoy?** Sí, y hay que hacerlo con la ventana del proyecto, no con la del maquetado. La convención ya está escrita y medida: `DIAS_DE_LA_TASA = 14` en `lib/negocio/indicadoresDeCitas.ts:282`, «catorce y no treinta… la ventana trasera del barrido son catorce días, así que más allá de eso la proporción de citas congeladas crece». Un botón de «30 días» en esta pantalla mostraría cohortes que el sistema dejó de mirar.
...
**3 · Las agendas por creativo.** Cruzando con `negocio.citas` por `contacto_id`, en la ventana y aplicando el piso del proyecto (`PISO_DE_UNA_TASA = 10`, `lib/negocio/indicadoresDeCitas.ts:272`):
...
**7 · El piso y la ventana son los del proyecto, no los del maquetado.** `PISO_DE_UNA_TASA = 10` (`lib/negocio/indicadoresDeCitas.ts:272`) y `DIAS_DE_LA_TASA = 14` (:282). Con eso, de los nueve creativos de la ventana sólo cuatro tienen tasa; los otros cinco tienen conteo y `null`, no cero. Y no hay que ofrecer «30 días»: más allá de catorce entran cohortes que el barrido ya dejó de refrescar.
