...
· Botón «Plan de acción» `#slPlanBtn` (SalesView.jsx:23). **Es un botón muerto.** Un grep sobre todo el código fuera de `.next/` devuelve exactamente una aparición de `slPlanBtn`: la del propio JSX. `lib/aios/period-controls.js:40` engancha `lpPlanBtn` (Leads Portal) y ningún otro. El botón se pinta, se puede hacer clic, y no pasa nada.
...
- **¿Se puede reemplazar hoy?** PARCIALMENTE, y solo dos de las cinco columnas. **Agendadas: SÍ.** Medido sobre la ventana de 14 días con el mismo filtro que ya usa `tasaDeCancelacion` (alcanzable = `ghl_calendario_id is not null`, no descartado = sin etiqueta de `ETIQUETAS_DE_DESCARTE`), las citas por closer son: `0peGoq7VvFqnDGA7gxtX` 47, `ryxDLpnsr1ERnlFNeSfz` (Jorge Veramendi) 21, `h0GyOecIxLdARqhzT5mZ` 5, sin asignar 2. Dos de los tres closers pasan el piso de 10; el tercero no. Sin el filtro serían 94 / 45 / 9 / 14 sobre 162 citas de la ventana, de las cuales 12 no tienen calendario y 82 pertenecen a contactos descartados. **Asistieron: NO** — `citas.asistio` es NULL en las 317 filas. **Ventas, Cierre, Revenue: NO** — cero filas de venta en toda la base. El subtítulo «ICP alto asignado» también es inventado: no existe ninguna tabla ni columna que asigne contactos a un closer por tramo de ICP.
...
- **Finge ser:** Los dos pasos finales del embudo ejecutivo, declarados propiedad de Sales: `{k:'asistidas', t:'Citas asistidas', own:'Sales'}` y `{k:'ventas', t:'Ventas', own:'Sales'}`. Son 10 filas de constantes con 30 valores de Sales en total. Para el periodo por omisión ('7d', línea 187) dice: 36 asistidas, 11 ventas, $27,940 de revenue.
...
**Las citas son el dato más sólido que Sales puede consumir.** 317 filas, del 2026-08-12 al 2026-09-19, con `crm_asignado_a` poblado en 183 (los 134 restantes están sin asignar). En los últimos 14 días, 162 citas ya pasadas; aplicando el filtro de `tasaDeCancelacion` quedan 75 alcanzables y no descartadas (12 sin `ghl_calendario_id`, 82 de contactos con etiqueta de descarte). Por closer, alcanzables y no descartadas en la ventana: 47 / 21 / 5 / 2 sin asignar. Estados del CRM sobre las 317: `cancelled` 160, `confirmed` 154, `noshow` 3.

**El ICP es real y pasa el piso con holgura.** El campo «Puntaje | ICP» (`9HXxl5DW6aayQgKUPiOS`, resuelto por nombre en `negocio.campos_del_crm`) tiene valor en **464 de 584 contactos, y 242 dentro de la ventana de 14 días**. Es el único insumo de calidad de lead que Sales puede usar hoy sin inventar nada. Ojo: la columna `negocio.contactos.score` es NULL en las 584 filas — el ICP no vive ahí, vive en el JSON del CRM.

**El territorio y la etapa.** `negocio.contactos`: 584 filas, 279 del territorio `closer` (129 creados en los últimos 14 días), 280 del `setter`, 25 sin territorio. La columna `etapa` está poblada en **6 contactos de 584** — los mismos 6 que tienen resultado: `seguimiento` 3, `no_show` 2, `descalificado` 1. Ningún contacto en etapa `ganado`.
...
· Campos del CRM resueltos POR NOMBRE en `negocio.campos_del_crm`: «Forma de pago venta» (`eUVX9ZlZsAHIwPr07hdR`) **0 de 584 contactos**; «forma_de_pago» (`k8u7GHpFDqBAW91q7hux`) 0; «Método de pago» (`fjUxq7EHNV6tSH77q5GG`) 0; «Cuota Inicial» (`YATkJYBm7J2FxTuKOMRv`) 0; «Cuotas» (`eesx5OAqxpkloIal25tb`) 0; «Cuota 2», «Cuota 3» y «Cuota 4» —los tres de tipo MONETORY, los únicos campos monetarios de toda la subcuenta— **0, 0 y 0**; «Estado del Producto» 0; «Política de Pagos» 0; «Etapa del Lead» 0; «Lead Score» 0.
· Etiquetas de contacto: un censo completo de `unnest(etiquetas)` sobre las 584 filas. Las 30 etiquetas más frecuentes no incluyen ninguna de venta. Un `similar to '%(vent|compr|pag|cliente|clos|gan|cerr|deal|win)%'` devuelve solo `zona_closer` (292, es territorio) y `afiliado ghl pago` (2, que es otra cosa).
· Oportunidades de GoHighLevel: no se ingestan. No hay tabla, no hay llamada al endpoint.
Conclusión: **la venta no existe bajo ningún otro nombre.** Tiene que venir del registro manual del closer, exactamente como dice el §5.4, y ese registro lleva 15 días abierto con cero ventas anotadas.

**2 · La asistencia. Existe el mecanismo desde HOY, y por eso no tiene ni una fila.**
`citas.asistio` es NULL en las 317 filas, y `resultados.cita_id` es NULL en las 7. No es abandono: la migración `db/migraciones/049_si_se_presento_a_la_cita.sql` —que agrega `citas.asistio` (líneas 46-47) y `resultados.cita_id` (líneas 65-66)— figura en `migraciones.migraciones_aplicadas` con fecha **2026-09-14T21:49**, o sea hoy. El escritor ya está en `lib/negocio/avanzar.ts:248`, que hace `.set({ asistio: lo.cita.asistio })` cuando el closer elige a qué cita se refiere. El encabezado de esa migración deja medido por qué no se puede leer del CRM: el campo de asistencia de GoHighLevel está poblado en 3 de 1 052 citas, y sus campos personalizados de asistencia en 0 de 316. Mi propia medición lo confirma desde otro ángulo: `estado_ghl='noshow'` son 3 filas de 317.
Hay una señal parcial que conviene no confundir con un dato: la etiqueta de contacto `noshow` está en **40 contactos**, bastantes más que los 3 del estado del CRM. Pero es de CONTACTO, no de CITA —un contacto con dos citas no se puede resolver— y en la ventana de 14 días solo toca 4 contactos y 5 citas, muy por debajo del piso de 10. Sirve como pista, no como denominador.

**3 · La llamada de venta. Cero grabaciones, cero transcripciones, cero auditoría.**
El §16.1 (línea 1072) declara «Auditoría de llamadas de venta» entre las capacidades existentes. Medido: `negocio.llamadas` tiene **0 filas**. `negocio.analisis_del_agente` tiene 46 filas, todas de agentes de TEXTO: `chat_post_agenda` 41 y `chat_pre_agenda` 5 — ninguna de una llamada de closer. En el CRM, el campo «Audio de llamada» (`t0aZuoISOYQftQNVLQaI`, FILE_UPLOAD) tiene valor en **0 de 584 contactos**; «Transcripcion de Elevenlabs» en 4 (y esas son del agente de voz, no del closer). El §16.1 sobreestima lo que hay: hoy la auditoría cubre Conversation, no Sales.

**4 · El eslabón `sales_call_id` y `sale_report_id` del §5.2.**
`sale_report_id` es, de hecho, `resultados.id`, así que ese eslabón existe aunque sin filas de venta. `sales_call_id` no tiene tabla: `negocio.llamadas` está vacía y no hay nada que la pueble. El eslabón `appointment_id → sale_report_id` acaba de abrirse hoy con `resultados.cita_id` y todavía no tiene ninguna fila que lo recorra.

**5 · La asignación de contactos por ICP.**
El subtítulo «ICP alto asignado / ICP medio y bajo» de la tabla de closers no tiene fuente: no hay tabla, columna ni campo del CRM que asigne un closer por tramo de ICP. El ICP existe (464 contactos), la asignación existe (`closer_asignado`, 3 filas), pero la REGLA que las une no está escrita en ninguna parte.
...
`lib/negocio/indicadoresDeCitas.ts:332-346`. El comentario dice por qué: «como el nulo es el caso normal —nadie cerró el intento todavía, y ninguna cita anterior a la `049` lo va a tener nunca— la tasa diría que no se presenta casi nadie. Sería una cifra plausible, alarmante y falsa». Con 0 de 317 citas con asistencia, hoy ese denominador es cero y la tasa es `null`.
...
**Contar las citas sin el filtro de descarte.** En la ventana de 14 días, 82 de 162 citas pertenecen a contactos con etiqueta de descarte. Una tabla de closers que las cuente duplica las agendadas de todos y baja artificialmente cada tasa de cierre a la mitad. El commit `9931f4d` ya documenta este error exacto en otra cifra de este mismo producto.
...
SÍ, hoy: (a) una tabla de closers con la columna «Agendadas» real, sobre `negocio.citas` + `negocio.closer_asignado`, con el filtro de alcanzables/no descartadas y con tres filas en vez de dos — 47, 21 y 5 citas en la ventana, más 2 sin asignar; el tercer closer queda por debajo del piso y hay que decirlo. (b) Un conteo de ventas y un revenue que hoy valen CERO MEDIDO, con el rótulo de «reportado por el closer» y con el testigo de que hubo 7 resultados registrados (si no hubiera ninguno, sería `null`, no cero). (c) El cruce con el ICP, que tiene 242 contactos con puntaje en la ventana y pasa el piso con holgura.
NO, hoy: tasa de cierre (7 eventos < 10), asistencias y show rate (0 de 317 citas con `asistio`), motivos de pérdida (1 fila, y dice «Otro»), cualquier cosa sobre la llamada de venta (`negocio.llamadas` vacía, 0 grabaciones en el CRM), y la asignación por tramo de ICP (la regla no existe).
Y la conclusión honesta que ordena todo lo demás: **el cuello de botella de Sales no es técnico.** El esquema está, el escritor está (`avanzar.ts`), la comisión está configurada al 10% para los tres closers, el mapeo al CRM está, y desde hoy hasta la asistencia tiene dónde guardarse. Lo que falta es que alguien registre. Quince días de operación con 317 citas produjeron 7 resultados y ninguna venta. Hasta que esa proporción cambie, cualquier pantalla de Sales que muestre un número va a estar mostrando, otra vez, un número inventado.
