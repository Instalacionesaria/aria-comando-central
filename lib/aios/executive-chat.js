...
export function initExecutiveChat() {
  ...
  /* ===================== CHAT CON EXECUTIVE ===================== */
  (function(){
  /* AQUÍ HABÍA UN MAPA `NAMES` con los diez nombres de sección escritos a mano, y estaba VENCIDO:
     le faltaba `tools`, agregada después. El síntoma era el fallback `|| 'Executive'`, o sea el
     panel diciendo «respondiendo con datos de Executive» estando abierto en Tools. Con el alcance
     por persona eso además nombra una pantalla que quien mira puede no tener.

     Era la cuarta copia de la lista que `lib/autorizacion/secciones.ts` existe para evitar, y la
     única sin prueba que la cruzara — `GROUP`, en `shell.js`, sí tiene una y por eso `tools` no le
     faltó. En vez de agregar la undécima entrada y esperar a la duodécima, el nombre se lee del DOM
     de la propia fila del menú, que es de donde `irALaVista` ya lo saca. Sin lista que mantener. */
  const nombreDeLaFila = (fila) => fila?.querySelector('.n')?.textContent.trim() || '';
  ...
  function currentView(){
    ...
    /* Sin fila marcada NO se inventa `executive`: es una pantalla que quien mira puede no tener.
       Devolver vacío deja el panel sin nombre de sección, que es cierto. */
    return on ? on.dataset.view : '';
  }
  function refreshScope(){
    const v = currentView(), n = nombreDeLaFila(document.querySelector('.nav-item.on'));
    ...
    /* Sin sugerencias para esta pantalla, NINGUNA — antes caía en las de Executive, así que en
       Tools ofrecía «¿Llegamos a las 30 ventas?». Una sugerencia de otra pantalla es peor que
       ninguna: se lee como si el panel supiera de ésta. */
    document.getElementById('askChips').innerHTML =
      (SUGG[v] || []).map(q=>`<button class="ap-chip">${q}</button>`).join('');
  ...
  /* El nombre sale de la fila CLICKEADA y no de `.nav-item.on`: este manejador y el que marca la
     fila activa son dos escuchas del mismo clic, y el orden entre ellos no está garantizado. */
  document.querySelectorAll('.nav-item[data-view]').forEach(n=>
    n.addEventListener('click', ()=> { document.getElementById('askScope').textContent = nombreDeLaFila(n); }));
  })();
}
