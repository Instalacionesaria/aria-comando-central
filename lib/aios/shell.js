 ...
 * ninguna. Los ABREN cinco módulos —`creative.js`, `conversion.js`, `leads-portal.js`,
 * `executive-panel.js` y `period-controls.js`— y hasta hoy los CERRABA uno solo: `creative.js`
 * registraba los cinco oyentes (`recoClose`, `recoScrim`, `scrim`, `dwClose` y el `Escape`) al final
 * de su propio módulo.
 *
 * O sea que el día que Creative deje de ser una maqueta y su módulo se borre, alguien abre «Plan de
 * acción» en Conversion y el modal queda con su velo sobre toda la aplicación **sin botón, sin velo
 * y sin `Escape` que lo cierre**. Sólo recargar. Y sin un error en consola, porque no hay ninguna
 * llamada que falle: simplemente no hay nadie escuchando.
 *
 * `executive-panel.js:64` lo deja todavía más a la vista: hace `getElementById('dwClose').click()`,
 * o sea que depende literalmente del oyente que registraba otro módulo de otra pantalla.
 *
 * Van acá y no en `Overlays.jsx` —que sería lo natural en React— porque los cinco módulos abren
 * estos overlays imperativamente con `classList`, y un componente que controlara `className` desde
 * el estado pelearía contra ellos: React volvería a poner lo suyo en el próximo render y el panel
 * se cerraría o se abriría solo. Mientras el que abre sea imperativo, el que cierra también.
 *
 * `shell.js` es el dueño del armazón y corre SEGUNDO en `MODULOS`, antes que cualquiera de los cinco. */
