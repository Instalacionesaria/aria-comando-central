...
export function initCreative() {
  ...
  /* Los CINCO oyentes que cerraban `#drawer` y `#recoModal` se fueron a `lib/aios/shell.js`, que es
     el dueño del armazón. Vivían acá y los dos overlays son compartidos: otros cuatro módulos los
     abren sin registrar nada, así que borrar este archivo los dejaba sin forma de cerrarse — y sin
     un error en consola. El motivo largo está en `shell.js`, sobre `initOverlays`. */
  document.getElementById('recoBtn').addEventListener('click', renderReco);
  renderAll();
