...
/* El cierre del cajón es del armazón, no de este módulo. Se importa —y no se sintetiza un clic
   sobre `#dwClose`— por el motivo que está escrito donde se usa. */
import { cerrarElCajon } from './shell';
...
export function initExecutivePanel() {
  ...
  function open(x){
    ...
    document.querySelectorAll('#dwBody [data-jump]').forEach(el=>{
      /* `cerrarElCajon()` y no `getElementById('dwClose').click()`, que es lo que hacía hasta el
         2026-09-20. Sintetizar un clic sobre el botón de cierre depende de que exista ESE nodo y de
         que ALGUIEN le haya registrado un oyente —y el dueño de ese oyente es `shell.js`, otro
         módulo—. Si el nodo cambia de id o el oyente no llegó a registrarse, el `.click()` no lanza
         nada: simplemente no pasa nada, el cajón se queda abierto y la vista salta por detrás.
         *
         Es el mismo modo de fallo que `closeReco()` en `conversion.js`, en su forma callada: allá
         el nombre no existía y lanzaba `ReferenceError`; acá el nombre existe y no hace nada. */
      el.onclick = ()=>{ cerrarElCajon();
