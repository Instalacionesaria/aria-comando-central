/* Portado de aios-command-center_1.html — líneas 3938-4582 del original.
   La única importación del archivo, y su porqué, están en el sitio donde se usa: buscá `cerrarElModal`. */
...
export function initConversion() {
  ...
  document.getElementById('cvRecoBtn').addEventListener('click', ()=>{
    ...
    document.querySelectorAll('.plan-item').forEach(el=>{
      /* `cerrarElModal` y no `closeReco()`, que **no está definido en ningún archivo de la
         aplicación** — sólo existía como función global del prototipo HTML, que no se carga.
         Clicar una de estas tres filas lanzaba `ReferenceError`, el `openStep()` de al lado
         **nunca se ejecutaba** y el modal se quedaba encima con su velo. No fallaba nada
         visible: el cajón sencillamente no se abría. */
      el.onclick = ()=>{ cerrarElModal(); openStep(el.dataset.goto); };
    });
